...
export class RasterViewer {
   ...
   *   source-YX plane (no HTTP `data_url`) which the cache then transposes.
   */
  constructor(host, options = {}) {
    this.host = host;
    this.instanceId = ++viewerInstanceCounter;
    this.options = options;
    this.loadSourcePlane = options.loadSourcePlane ?? null;
  ...
  async load(descriptor) {
    ...
    this.xyPlots.clear();
    if (typeof this.loadSourcePlane !== 'function') {
      throw new Error('RasterViewer requires loadSourcePlane to fetch OME-Zarr planes');
    }
    this.planeCache = new PlaneCache(descriptor, detail => {
      this.performanceMetric('plane-fetch', detail);
    }, this.loadSourcePlane);
